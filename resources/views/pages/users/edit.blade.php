...
<div class="row">
    <div class="col-12">
        <div class="page-title-box">
            ...
            <h4 class="page-title">{{ __('global.edit') }} {{ __('cruds.user.title_singular') }}</h4>
        </div>
    </div>
</div>
<!-- end page title -->
                ...
                <form action="{{ route("users.update", $user->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="form-group col-md-4">
                            ...
                            <input type="text" id="nome" name="nome" class="form-control {{ $errors->has('nome') ? 'has-error' : '' }}" value="{{ old('nome', $user->nome) }}">
                            @if($errors->has('name'))
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <input type="email" id="email" name="email" class="form-control {{ $errors->has('email') ? 'has-error' : '' }}" value="{{ old('email', $user->email) }}">
                            @if($errors->has('email'))
                                <div class="mt-1 require_error">
                                    {{ $errors->first('email') }}
                                </div>
                            ...
                            <div>
                                <select name="nivel" id="nivel" class="form-control" data-toggle="select2">
                                    @foreach($level as $key => $value)
                                        <option value="{{ $key }}" {{ old('nivel', $user->nivel) == $key ? 'selected' : '' }}>{{ $value }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @if($errors->has('nivel'))
                            ...
                            <div>
                                <select name="roles[]" id="roles" class="form-control" multiple="multiple" data-toggle="select2">
                                    @foreach($rolePermissions as $key => $permission)
                                        <option value="{{ $key }}" {{ (in_array($key, old('roles', [])) || isset($user) && $user->roles()->pluck('name', 'id')->contains($key)) ? 'selected' : '' }}>{{ $key }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @if($errors->has('roles'))
                            ...
                            <div>
                                <select name="permissions[]" id="permissions" class="form-control" multiple="multiple" data-toggle="select2">
                                    @foreach($permissions as $id => $permission)
                                        <option value="{{ $id }}" {{ in_array($id, old('permissions', $user->getAllPermissions()->pluck('name')->all())) ? 'selected' : '' }}>{{ $permission }}</option>
                                    @endforeach
                                </select>
                            </div>
